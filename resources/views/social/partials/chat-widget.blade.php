@if (config('network.enabled'))
    <div class="w-full rounded-md py-2 overflow-hidden">
        <div id="chat-box" class="w-full rounded-md bg-white mb-6 shadow-sm px-4 flex-col overflow-y-scroll pb-4" style="height: 45vh;">
                ...
                <div
                    ...
                    <div class="mr-2" style="min-width: 50px;">
                        @include('components.profile.avatar', [
                            'src' => $chatMessage['user']['avatar_path'],
                        ])
                    </div>
                    <div>
                        <div class="font-semibold">
                            ...
                            <span
                                class="ml-2 text-sm text-gray-500">{{ \Carbon\Carbon::parse($chatMessage['timestamp'])->addMinutes(12)->diffForHumans() }}</span>
                        </div>
        ...
        <div>
            <form action="{{ route('social.chat.send_message') }}" method="POST">
                @csrf
                <x-text-input id="chat_input" name="chat_input" type="text" class="mt-1 block w-full" autofocus
